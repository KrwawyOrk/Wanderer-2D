...
void StaticMapItem::Draw( void )
{
	...
	int x = static_cast<int>( flposition_x - Globals::camera->GetCameraX() );
	...
	{
		SDL_Rect src;
		src.x = Globals::tilesize * int(((SDL_GetTicks() / Globals::spriteAnimationSpeed) % 6 ));
		src.y = 0;
		src.w = Globals::tilesize;
		src.h = spriteHeigh;

		m_sprite.Draw( Globals::screen, x, y, &src );
	}
}
